...
<Tabs>
  ...
  <TabItem label="Cloudflare AI Gateway">
    Cloudflare AI Gateway routes requests to upstream providers through Cloudflare with caching, rate limiting, and observability.

    1. Visit [dash.cloudflare.com](https://dash.cloudflare.com)
    2. Note your **Account ID** and create an AI Gateway (note the **Gateway ID**, defaults to `default`)
    3. Create an API token with AI Gateway permissions
  </TabItem>
  <TabItem label="Cloudflare Workers AI">
    Workers AI runs open models directly on Cloudflare's network.
  ...
  <TabItem label="Custom">
    Use any OpenAI-compatible endpoint (LM Studio, Ollama, self-hosted APIs). No API key needed for local setups; run `msh --add-model` to configure. See [AI Providers](/features/providers/) for details.
  </TabItem>
</Tabs>
